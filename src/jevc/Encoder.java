...
public class Encoder {
    ...
    private int DEFAULT_QUALITY_FACTOR = 80;
    ...
    public Encoder(YCbCrImage image, String outfile) {
        this.image = image;
    ...
    public void compress() throws IOException {
        ...
        ArrayList<Block> blocks = image.PerformBlockSplitting();
        RunLengthBlock rleBlock;
        ArrayList<Block> DebugColorBlocks = new ArrayList<Block>();
        ArrayList<Block> DebugDCTBlocks = new ArrayList<Block>();
        ArrayList<Block> DebugQBlocks = new ArrayList<Block>();
        ...
        writeTrailerSection(outputStream);
    ...
    private void writeHeaderSections(BufferedOutputStream outputStream) throws IOException {
        outputStream.write(SOI);
        writeAPP0Segment(outputStream);
        writeCOMSegment(outputStream);
        writeDQTSegment(outputStream);
        writeSOF0Segment(outputStream);
    ...
    private void writeSOF0Segment(BufferedOutputStream outputStream) throws IOException {
        ...
        SOF0Segment[5] = (byte) ((image.getHeight() >> 8) & 0xff);
        SOF0Segment[6] = (byte) (image.getHeight() & 0xff);
        SOF0Segment[7] = (byte) ((image.getWidth() >> 8) & 0xff);
        SOF0Segment[8] = (byte) (image.getWidth() & 0xff);
        SOF0Segment[9] = (byte) 3; // number of components
        int[] horizSamplingFactors = {1,1,1};
        int[] vertSamplingFactors = {1,1,1};
        image.computeSamplingFactors(image.getSampling(), horizSamplingFactors, vertSamplingFactors);
        int index = 10;
    ...
    private void writeDHTSegment(BufferedOutputStream outputStream) throws IOException {
         ...
         *	   o HT information, 1 byte: bit 0..3 - number of this HT (0,1,2,3)
         *				 bit 4 - type of this HT (0=DC table, 1=AC table)
         *				 bit 5..7 - not used, must be zero
         *	   o Number of symbols, 16 bytes: number of symbols with codes of length 1,2..16.
         *				      The sum (let this sum be n) of these bytes is the
         *				      total number of codes, which must be <=256
         *	   o Symbols, n bytes (see above): table containing the symbols in order of increasing
